...
public static void main(String[] args) {
		...
		ReserveRoom reserv  = new ReserveRoom();
	...
	JPanel pnlReservationDate, pnlRoomType, pnlCheck_in_date, pnlCheck_out_date, pnlFreeRooms, pnlCustomer, pnlservice, pnlReport;
	JButton btnSubmit;
	//JComboBox comboList; 
	...
	public void addReserveRoom()
		{
			...
			labService = new JLabel("Service");
                        ...
                        text_reservation_date = new JTextField(10);
			textCheck_in_date = new JTextField(10); 
			textCheck_out_date = new JTextField(10); 
			final JTextArea ta_report = new JTextArea();
                        ta_report.setFont(new Font("Serif", Font.BOLD, 18));
                        ta_report.setText("hhsh");
			...
			pnlCheck_in_date.add(labCheck_in_date, BorderLayout.WEST); 
			pnlCheck_in_date.add(textCheck_in_date, BorderLayout.WEST); 
			pnlCheck_out_date.add(labCheck_out_date, BorderLayout.WEST); 
			pnlCheck_out_date.add(textCheck_out_date, BorderLayout.WEST);
			pnlRoomType.add(labRoomTypeId, BorderLayout.WEST);
			pnlRoomType.add(comboRoomType, BorderLayout.WEST);
			getRoomTypeId();
			...
			//getFreeRoom();
			//Add 
                        cp.add(pnlReservationDate, BorderLayout.WEST);
			cp.add(pnlCheck_in_date, BorderLayout.WEST);
			cp.add(pnlCheck_out_date, BorderLayout.WEST);
			cp.add(pnlRoomType, BorderLayout.WEST);
                        cp.add(pnlFreeRooms, BorderLayout.WEST);
			cp.add(pnlCustomer, BorderLayout.WEST);
			cp.add(pnlService);
			cp.add(new JScrollPane(pnlReport),BorderLayout.CENTER);
		...
		//public void getFreeRoom()
	{
		...
		try {
			...
			String sql_sel = "SELECT Room_Id, Room_Name " +
				...
				" AND Start_Date BETWEEN '" + textCheck_in_date.getText() + "' AND ' " + textCheck_out_date.getText() + " ' AND End_Date >= ' " + textCheck_in_date.getText() + " ')";
	...
	class SaveReservation implements ActionListener {
		...
		try {
			...
			String sql = "INSERT INTO Reservation (Room_Id, Reservation_Date, Customer_Id, Service_Category_Id, Start_Date, End_Date) " +
